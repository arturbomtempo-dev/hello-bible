...
export class BibleViewProvider implements vscode.WebviewViewProvider {
    ...
    resolveWebviewView(webviewView: vscode.WebviewView): void {
        ...
        const renderVerse = () => {
            ...
            webviewView.webview.html = this.getPage(this.getVerseBodyHtml(verse, isFavorite));
        };

        const load = async () => {
            webviewView.webview.html = this.getPage(this.getLoadingBodyHtml());

            try {
                verse = await this.bibleService.getDailyVerse();
                renderVerse();
            } catch {
                webviewView.webview.html = this.getPage(this.getErrorBodyHtml());
            }
        };
    ...
    private getPage(bodyHtml: string): string {
        return `
            <!doctype html>
            <html lang="pt-BR">
                <head>
    ...
    private getBookIconSvg(extraClass = ''): string {
        return `
            <svg
    ...
    private getVerseBodyHtml(verse: Verse, isFavorite: boolean): string {
    ...
    private getLoadingBodyHtml(): string {
        return `
            <div class="card">
                ${this.getBookIconSvg('is-loading')}
                <p class="status-text">Carregando versículo do dia&hellip;</p>
            </div>
        `;
    }

    private getErrorBodyHtml(): string {
        return `
            <div class="card">
                <svg
                    class="icon"
