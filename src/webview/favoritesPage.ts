...
function getStarIconSvg(): string {
    return `
        <svg
            class="icon"
            viewBox="0 0 24 24"
...
function getListHtml(favorites: FavoriteVerse[]): string {
...
export function getFavoritesPage(favorites: FavoriteVerse[], accentColor?: string): string {
    const sorted = [...favorites].sort((a, b) => b.favoritedAt.localeCompare(a.favoritedAt));
    const body = sorted.length === 0 ? getEmptyStateHtml() : getListHtml(sorted);
